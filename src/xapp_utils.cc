...
void XaPP::init(int msg_size){
  ...
  if (msg_size > RMR_BUFFER_SIZE or msg_size <= 0){
    ...
    std::string error_string = identifier + " : Error: RMR buffer size " + std::to_string(msg_size) + " is invalid . Must be between 1 and " + std::to_string(RMR_BUFFER_SIZE) ;
    error_string  +=  std::string(_xapp_name) + std::string(" on port ") + std::string(_proto_port);
    throw error_string; 
  }
  ...
  if ( (_rmr_ctx = rmr_init(_proto_port, msg_size, RMRFL_NONE)) == NULL){
    ...
    std::string identifier = __FILE__ +  std::string(", Line: ") + std::to_string(__LINE__) ; 
    std::string error_string = identifier + " : Error initializing the rmr context for app " ;
    error_string  +=  std::string(_xapp_name) + std::string(" on port ") + std::string(_proto_port);
    throw error_string;
  }
