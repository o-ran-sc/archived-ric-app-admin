...
ric_control_request::ric_control_request(void){
  ...
  for(int i = 0; i < NUM_CONTROL_REQUEST_IES; i++){
    ASN_SEQUENCE_ADD(&(ric_control_request->protocolIEs), &(IE_array[i]));
  }
...
ric_control_request::~ric_control_request(void){
  ...
  mdclog_write(MDCLOG_INFO, "Freeing E2AP Control Request object memory");
  ...
  mdclog_write(MDCLOG_INFO, "Freed E2AP Control Request object mempory");
...
bool ric_control_request::set_fields(InitiatingMessage_t *initMsg, ric_control_helper &dinput){
  ...
  *ranfunction_ie = dinput.func_id;
  ...
  // ies_riccallprocessid->value.present = RICcontrolRequest_IEs__value_PR_RICcallProcessID;
